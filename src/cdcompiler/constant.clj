(ns cdcompiler.constant
  (:require [cdcompiler.const-pool :refer :all]))
...
(defmethod constant
  ...
  (let [r1 (.read fis)
    ...
    (pool-add (str "#" const-ref ":#" name-ref))))
...
(defmethod constant
  ...
  (let [r1 (.read fis)
    ...
    (pool-add (str "#" method-ref ".#" type-ref))
    ))
...
(defmethod constant
  ...
  (let [r1 (.read fis)
    ...
    (println "字段所在类索引地址:" field-ref)
    (println "字段描述索引地址:" type-ref)
    (pool-add (str "#" field-ref ".#" type-ref))))
...
(defmethod constant
  ...
  (let [r1 (.read fis)
        ...
        str-ref (+ (* 100 r1) r2) ]
    (println "String_info")
    (println "字符串索引地址:" str-ref)
    (pool-add (str "#" str-ref))))
...
(defmethod constant
  ...
  (let [r1 (.read fis)
        ...
        clz-ref (+ (* 100 r1) r2) ]
    (println "Class_info")
    (println "类型索引地址:" clz-ref)
    (pool-add (str "#" clz-ref))))
